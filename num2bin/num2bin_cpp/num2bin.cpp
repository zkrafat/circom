...
#include "circom.hpp"
#include "calcwit.hpp"
void Num2Bin_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Num2Bin_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "Num2Bin";
...
Fr_sub(&expaux[3],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 0)],&circuitConstants[2]); // line circom 11
...
PFrElement aux_dest = &lvar[2];
// load src
Fr_mul(&expaux[0],&lvar[2],&circuitConstants[3]); // line circom 13
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 4]); // line circom 16
...
Num2Bin_0_create(1,0,ctx,"main",0);
Num2Bin_0_run(0,ctx);
}
